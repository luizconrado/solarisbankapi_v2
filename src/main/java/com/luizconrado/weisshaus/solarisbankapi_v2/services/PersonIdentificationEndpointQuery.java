...
public class PersonIdentificationEndpointQuery {


    public List<PersonIdentification> getPersonIdentifications(AuthenticationToken token, int pageSize, int pageNumber, Person person) {
        ...
        List<PersonIdentification> personIdentifications = new ArrayList<>();
...
//            System.out.println(response.getStatus());
            ...
            if (response.getStatus() >= 200 && response.getStatus() < 300) {
                ...
                try {
                    ...
                    System.out.println("getPersonIdentifications Failed -> jsonNode = mapper.readTree(responseString)");
                    e.printStackTrace();
                ...
                try {
                    ...
                    List<PersonIdentification> personTaxIdentificationsTemp = mapper.readValue(responseString, new TypeReference<List<PersonIdentification>>() {
                    });

                    if (personTaxIdentificationsTemp.size() > 0) {
                        personTaxIdentificationsTemp.forEach(p -> p.setPersonId(person.getId()));
                        personIdentifications.addAll(personTaxIdentificationsTemp);
                    }
